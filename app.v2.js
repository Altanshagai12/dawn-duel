...
// src/game/ObjectiveViews.js
function colorOf(site) {
  return Number.parseInt(String(site.color || "#ffffff").slice(1), 16);
}
var ObjectiveViews = class {
  constructor(scene) {
    this.scene = scene;
    this.buffSites = /* @__PURE__ */ new Map();
  ...
  createBuffSite(site) {
    ...
    const halo = this.scene.add.circle(0, 0, MAP.buffPocketRadius, color, 0.045).setStrokeStyle(4, color, 0.52);
    const core = this.scene.add.circle(0, 0, 31, color, 0.16).setStrokeStyle(3, 16777215, 0.72).setBlendMode(Phaser.BlendModes.ADD);
    ...
    const meterBg = this.scene.add.rectangle(0, 65, 76, 6, 132871, 0.82);
    const meter = this.scene.add.rectangle(-38, 65, 76, 4, color, 1).setOrigin(0, 0.5);
    root.add([halo, core, label, meterBg, meter]);
    this.scene.tweens.add({ targets: [halo, core], alpha: 0.24, scale: 1.06, duration: 1e3, yoyo: true, repeat: -1 });
  ...
  apply(sites = [], now = 0) {
    for (const site of sites) {
      ...
      view.root.setAlpha(ready ? 1 : 0.38);
...
var GameScene = class extends Phaser.Scene {
  ...
  drawMap() {
    ...
    g.lineStyle(MAP.laneWidth, 7906454, 0.035).lineBetween(MAP.blueCoreX, MAP.blueCoreY, MAP.redCoreX, MAP.redCoreY);
    g.lineStyle(4, 15255673, 0.18).lineBetween(MAP.blueCoreX, MAP.blueCoreY, MAP.redCoreX, MAP.redCoreY);
    for (const site of MAP.campSites) {
      const approach = campApproach(site);
      const color = site.side ? 16741499 : 6153951;
      g.lineStyle(MAP.campPathRadius * 2, color, 0.045).lineBetween(approach.x, approach.y, site.x, site.y);
      g.fillStyle(color, 0.05).fillCircle(site.x, site.y, MAP.campPocketRadius);
      const angle = Math.atan2(approach.y - site.y, approach.x - site.x);
      const gap = 0.52;
      g.lineStyle(3, color, 0.3).beginPath().arc(site.x, site.y, MAP.campPocketRadius, angle + gap, angle + Math.PI * 2 - gap).strokePath();
      ...
      g.lineStyle(2, color, 0.24).lineBetween(site.x + normalX, site.y + normalY, approach.x + normalX, approach.y + normalY).lineBetween(site.x - normalX, site.y - normalY, approach.x - normalX, approach.y - normalY);
    }
    for (const site of MAP.buffSites) {
      const approach = campApproach(site);
      const color = Number.parseInt(site.color.slice(1), 16);
      g.lineStyle(MAP.buffPathRadius * 2, color, 0.055).lineBetween(approach.x, approach.y, site.x, site.y);
    }
...
var UIController = class {
  ...
  update(snapshot) {
    ...
    buffStatus.textContent = `${this.t().surge} \xB7 ${buffRemaining}s`;
  ...
  updateAnnouncement(snapshot, player) {
    ...
    else if ((player.surgeUntil || 0) > snapshot.now && player.surgeUntil > this.lastSurgeUntil) text = `${this.t().surge} \xB7 30s`;
