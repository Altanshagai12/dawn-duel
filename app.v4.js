...
var EntityViews = class {
  ...
  update(time, delta = 16) {
    for (const [id, view] of this.items) {
      ...
      let facingX = dx;
      let facingY = dy;
      const factor = local ? Math.hypot(dx, dy) > 65 ? 0.42 : 0.08 : 0.28;
      const correction = 1 - (1 - factor) ** (delta / (1e3 / 60));
...
var AimView = class {
  ...
  update(playerView, input2, preview) {
    ...
    g.clear();
    if (!playerView || !preview && !input2?.attack) return;
...
async function boot() {
  ...
  bridge.attack = (active) => {
    input.state.attack = input.enabled && active;
  };
