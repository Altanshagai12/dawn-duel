...
- When a landscape-only game runs inside Usion's portrait-locked mobile WebView, request the native Screen Orientation API first, then rotate the entire game viewport as a fallback. Remap joystick vectors through the inverse rotation and test both layout and controls; rotating visuals alone breaks input axes.
