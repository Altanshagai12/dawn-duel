...
// src/game/inputTimeline.js
var INPUT_TIMELINE = Symbol("dawnDuelInputTimeline");
var MAX_AGE_MS = 1e3;
var MAX_ENTRIES = 64;
var MAX_REPLAY_MS = 300;
...
var InputTimeline = class {
  ...
  record(input2, sentAt = this.clock()) {
    if (!validSequence(input2?.seq) || !Number.isFinite(sentAt)) return false;
    const last = this.entries.at(-1);
    if (last && input2.seq <= last.seq) return false;
    this.entries.push({ seq: input2.seq, at: sentAt, moveX: input2.moveX || 0, moveY: input2.moveY || 0 });
...
function facingRow(x, y, previous = 4) {
  if (!Number.isFinite(x) || !Number.isFinite(y) || Math.hypot(x, y) < 1e-3) return previous;
...
var MotionClock = class {
  ...
  push(serverMs, receivedMs) {
    ...
    if (this.offset === null) this.offset = observed;
      ...
      this.jitter += (variation - this.jitter) * 0.15;
      this.offset = observed < this.offset ? observed : this.offset + (observed - this.offset) * 0.01;
...
var EntityMotion = class {
  ...
  accept(entity, serverMs, receivedMs, force = false) {
    ...
    if (this.snap) {
      ...
      this.reconcile = {
        ...this.authoritative,
        inputSeq: entity.inputSeq,
        inputAgeMs: entity.inputAgeMs,
        receivedMs
      };
    }
...
function moveView(view, { local, input: input2, playing, now, clientMs, renderMs, delta, structures }) {
  ...
  if (local) {
    ...
    if (motion.reconcile) {
      ...
      const timeline = input2?.[INPUT_TIMELINE];
      replayed = timeline?.forEachReplaySegment(
        pending.inputSeq,
        pending.inputAgeMs,
        clientMs,
        (sample, segmentMs, elapsedMs) => {
          point = predictMove(point, sample, entity, now + elapsedMs / 1e3, segmentMs, structures);
        }
      ) || false;
      motion.predicted = point;
      motion.reconcile = null;
    ...
    if (moving && clientMs - motion.receivedMs <= 300) {
      ...
      motion.moveX = next.x - root.x;
      motion.moveY = next.y - root.y;
      root.x = next.x;
      root.y = next.y;
    }
    ...
    const correction = Math.min(error * alpha, predictionSpeed(entity, now) * delta / 1e3 * 0.25);
...
// src/game/facing.js
var POSE_MS = 300;
var DIRECTIONAL = /* @__PURE__ */ new Set(["precision", "volley", "repulse", "emberLine", "shadowStep", "moonSnare"]);
var MIRROR_WEST = /* @__PURE__ */ new Set(["hina", "scarlett"]);
var FacingState = class {
  constructor(row = 4) {
    this.row = row;
    this.reset();
  }
  reset(entity) {
    ...
    this.skillAt = -Infinity;
    this.skillId = null;
    this.latestAt = -Infinity;
    this.priority = 0;
    this.untilMs = -Infinity;
  }
  confirm(at, angle, priority, serverNow, clientMs) {
    if (!Number.isFinite(at) || !Number.isFinite(angle) || at < 0 || at > serverNow + 1e-3) return;
    if (at < this.latestAt || at === this.latestAt && priority < this.priority) return;
    const remaining = POSE_MS - Math.max(0, serverNow - at) * 1e3;
    if (remaining <= 0) return;
    this.latestAt = at;
    this.priority = priority;
    this.angle = angle;
    this.untilMs = clientMs + remaining;
  }
  observeAttack(entity, serverNow, clientMs) {
    ...
    if (!Number.isFinite(angle) && Number.isFinite(entity.attackAimX) && Number.isFinite(entity.attackAimY) && Math.hypot(entity.attackAimX, entity.attackAimY) > 1e-3) {
  ...
  observeSkill(effect, serverNow, clientMs) {
    ...
    const at = effect.castAt ?? effect.expiresAt - 0.45;
    ...
    if (effect.skillId === "shadowStep") {
      ...
      if (!Number.isFinite(x) || !Number.isFinite(y) || Math.hypot(x, y) < 1e-3) return;
...
function heroAnimationFrame(hero, row, animationMs) {
...
var SkillEffects = class {
  ...
  show(effect, now, clientNow = now) {
    ...
    slot.until = ground ? Math.min(effect.expiresAt || now + 2.5, now + 2.5) : clock + 0.4;
    slot.size = size;
    slot.ground = ground;
...
var VERBS = {
  mn: {
    ...
    repulse: "\u0422\u04AE\u041B\u0425\u042D\u0425",
    emberLine: "\u0413\u0410\u041B\u042B\u041D \u0411\u04AE\u0421",
    cinderFocus: "\u0425\u04AE\u0427\u0422\u042D\u0419 3 \u0421\u0423\u041C",
    shadowStep: "DASH + \u0425\u0423\u0423\u041B\u0411\u0410\u0420",
    moonSnare: "\u0414\u0423\u0423\u0421\u0413\u0410\u0425 \u0421\u0423\u041C"
  },
  en: {
    precision: "MARK SHOT",
    volley: "RECOIL VOLLEY",
    aegis: "SHIELD",
    repulse: "PUSH SHOT",
    emberLine: "FIRE FIELD",
    cinderFocus: "3 POWER SHOTS",
    shadowStep: "DASH + CLONE",
    moonSnare: "FINISHER SHOT"
  }
};
function skillVerb(id, language = "mn") {
  return (VERBS[language] || VERBS.en)[id] || "";
}
function passiveDetail(hero, language = "mn") {
...
function activeStatusLabels(player, now, language = "mn") {
  ...
  if (player.cinderUntil > now && player.cinderCharges > 0) result.push(`${mn ? "\u25B2 \u0425\u04AE\u0427\u0422\u042D\u0419" : "\u25B2 POWER"} \xD7${player.cinderCharges}`);
  if (player.riposteReady || player.riposteUntil > now && player.riposteDamage > 0) result.push(mn ? "\u25C6 \u0425\u0410\u0420\u0418\u0423 \u0426\u041E\u0425\u0418\u041B\u0422" : "\u25C6 RIPOSTE READY");
  return result;
}
function skillLiveDetail(player, index, stats, language = "mn") {
  const skill = HEROES[player.hero].skills[index], mn = language === "mn";
  const round3 = (n) => Math.round(n * 10) / 10;
  const damage = (n) => round3(n * stats.skillDamage);
  const descriptions = {
    precision: mn ? `${damage(skill.damage)} \u0433\u044D\u043C\u0442\u044D\u043B \u2192 ${skill.markSeconds}s \u0442\u044D\u043C\u0434\u044D\u0433. \u0414\u0430\u0440\u0430\u0430\u0433\u0438\u0439\u043D \u0446\u043E\u0445\u0438\u043B\u0442 +${damage(skill.markDamage)}.` : `${damage(skill.damage)} damage \u2192 ${skill.markSeconds}s mark. Next hit +${damage(skill.markDamage)}.`,
    volley: mn ? `${skill.count} \xD7 ${damage(skill.damage)} \u0441\u0443\u043C; ${skill.recoil} \u0437\u0430\u0439\u0434 \u0443\u0445\u0430\u0440\u043D\u0430; ${Math.round(skill.slow * 100)}% \u0443\u0434\u0430\u0430\u0448\u0440\u0443\u0443\u043B\u043D\u0430.` : `${skill.count} \xD7 ${damage(skill.damage)} bolts; recoil ${skill.recoil}; ${Math.round(skill.slow * 100)}% slow.`,
    aegis: mn ? `${skill.shield} \u0431\u0430\u043C\u0431\u0430\u0439 ${skill.duration}s. \u0428\u0438\u043D\u0433\u044D\u044D\u0441\u044D\u043D \u0433\u044D\u043C\u0442\u043B\u044D\u044D\u0441 \u0434\u0430\u0440\u0430\u0430\u0433\u0438\u0439\u043D \u0446\u043E\u0445\u0438\u043B\u0442/\u0415 +${skill.riposteCap} \u0445\u04AF\u0440\u0442\u044D\u043B.` : `${skill.shield} shield for ${skill.duration}s. Absorbed damage empowers next attack/E by up to ${skill.riposteCap}.`,
    repulse: mn ? `${damage(skill.damage)} \u0433\u044D\u043C\u0442\u044D\u043B; ${skill.knockback} \u0437\u0430\u0439\u0434 \u0442\u04AF\u043B\u0445\u044D\u043D\u044D; ${Math.round(skill.slow * 100)}% \u0443\u0434\u0430\u0430\u0448\u0440\u0443\u0443\u043B\u043D\u0430.` : `${damage(skill.damage)} damage; push ${skill.knockback}; ${Math.round(skill.slow * 100)}% slow.`,
    emberLine: mn ? `${skill.windup}s \u0430\u043D\u0445\u0430\u0430\u0440\u0443\u0443\u043B\u0433\u0430 \u2192 ${skill.pulses} \xD7 ${damage(skill.damage)} \u0433\u044D\u043C\u0442\u044D\u043B. \u0411\u04AF\u0441\u044D\u044D\u0441 \u0433\u0430\u0440\u0432\u0430\u043B \u0437\u0430\u0439\u043B\u043D\u0430.` : `${skill.windup}s warning \u2192 ${skill.pulses} \xD7 ${damage(skill.damage)} damage. Leave the field to avoid it.`,
    cinderFocus: mn ? `${skill.duration}s \u0445\u0443\u0440\u0434 +${Math.round(skill.speedBonus * 100)}%; ${skill.charges} \u0441\u0443\u043C \u0442\u0443\u0441 \u0431\u04AF\u0440 +${damage(skill.bonusDamage)}.` : `+${Math.round(skill.speedBonus * 100)}% speed for ${skill.duration}s; ${skill.charges} shots each gain ${damage(skill.bonusDamage)}.`,
    shadowStep: mn ? `${skill.distance} \u0437\u0430\u0439\u0434 \u04AF\u0441\u044D\u0440\u043D\u044D. \u04AE\u043B\u0434\u0441\u044D\u043D \u0445\u0443\u0443\u043B\u0431\u0430\u0440 ${skill.cloneSeconds}s-\u0434 ${skill.cloneShots} \xD7 ${damage(skill.cloneDamage)} \u0431\u0443\u0443\u0434\u043D\u0430.` : `Dash ${skill.distance}. A clone stays for ${skill.cloneSeconds}s, firing ${skill.cloneShots} \xD7 ${damage(skill.cloneDamage)}.`,
    moonSnare: mn ? `${damage(skill.damage)} + \u0430\u043B\u0434\u0441\u0430\u043D HP-\u0438\u0439\u043D ${round3(skill.missingHpRatio * stats.skillDamage * 100)}% (\u0434\u044D\u044D\u0434 +${damage(skill.missingHpCap)}). \u0428\u0430\u0440\u0445\u0430\u0434\u0441\u0430\u043D \u0431\u0430\u0439\u0434 \u0445\u04AF\u0447\u0442\u044D\u0439.` : `${damage(skill.damage)} + ${round3(skill.missingHpRatio * stats.skillDamage * 100)}% missing HP (max +${damage(skill.missingHpCap)}). Stronger against wounded targets.`
  };
  const range = skill.range || skill.distance;
  return `${descriptions[skill.id]} ${range ? `${mn ? "\u0417\u0430\u0439" : "Range"} ${range} \xB7 ` : ""}${mn ? "\u0425\u04AF\u043B\u044D\u044D\u043B\u0442" : "Cooldown"} ${round3(skill.cooldown * stats.cooldown)}s`;
}

// src/game/CombatCallouts.js
var CombatCallouts = class {
  constructor(scene, capacity = 8) {
    this.slots = Array.from({ length: capacity }, () => ({
      text: scene.add.text(0, 0, "", {
        ...
        stroke: "#041112",
        strokeThickness: 5
      }).setOrigin(0.5).setDepth(780).setVisible(false),
      until: 0
    }));
  }
  reset() {
  ...
  show(effect, now) {
    ...
    slot.until = now + 1;
    slot.y = effect.y - 99;
    slot.ownerId = effect.ownerId;
    slot.follow = effect.kind === "skillCast" || effect.power === "aegis";
    slot.x = effect.x;
  ...
  update(now, entities) {
    for (const slot of this.slots) {
      ...
      if (now >= slot.until || slot.ownerId && !owner) {
        slot.text.setVisible(false);
        continue;
      }
      const age = now - slot.started;
      slot.text.setPosition(slot.follow && owner ? owner.root.x : slot.x, (slot.follow && owner ? owner.root.y - 99 : slot.y) - age * 12).setAlpha(Math.min(1, (slot.until - now) / 0.25));
    }
  }
};
...
function createStatusView(scene, root) {
  ...
  const aegisRing = scene.add.circle(0, 13, 30, 6016767, 0).setScale(1, 0.52).setStrokeStyle(3, 6016767, 0.85).setVisible(false);
  const tempoRing = scene.add.circle(0, 13, 37, 16742473, 0).setScale(1, 0.52).setStrokeStyle(3, 16742473, 0.85).setVisible(false);
  const textStyle = { fontFamily: "system-ui", fontSize: "10px", fontStyle: "bold", color: "#fff6d2", stroke: "#051013", strokeThickness: 4 };
  const aegisGlyph = scene.add.text(-18, 26, "\u25C6", { ...textStyle, fontSize: "13px", color: "#75dbff" }).setOrigin(0.5).setVisible(false);
  const tempoGlyph = scene.add.text(18, 26, "\u2739", { ...textStyle, fontSize: "15px", color: "#ff9963" }).setOrigin(0.5).setVisible(false);
  const conditions = scene.add.text(0, -69, "", textStyle).setOrigin(0.5).setVisible(false);
...
function updateStatusView(status, player, now) {
  ...
  status.aegisRing.setVisible(aegis);
  status.aegisGlyph.setVisible(aegis);
  status.tempoRing.setVisible(tempo);
  status.tempoGlyph.setVisible(tempo);
  const text = activeStatusLabels(player, now, globalThis.document?.documentElement.lang || "mn").join(" \xB7 ");
  if (status.conditions.text !== text) status.conditions.setText(text);
  status.conditions.setVisible(Boolean(text));
}

// src/game/recentEffects.js
function rememberEffect(seen, id, capacity = 512) {
...
var EntityViews = class {
  ...
  apply(snapshot) {
    ...
    for (const entity of entities) {
      ...
      view.facing || (view.facing = new FacingState(view.row));
  ...
  update(time, delta = 16) {
    ...
    this.skillEffects?.update(visualNow, clientMs / 1e3);
    this.shotEffects?.update(clientMs / 1e3);
    this.callouts?.update(clientMs / 1e3, this.items);
    for (const [id, view] of this.items) {
      ...
      const moving = !view.justSnapped && Math.hypot(local ? view.motion.moveX : dx, local ? view.motion.moveY : dy) > delta / 1e3;
      const steering = local && Math.hypot(input2.moveX || 0, input2.moveY || 0) > 0.02;
      const moveX = local ? steering ? moving ? view.motion.moveX : input2.moveX : 0 : moving ? dx : 0;
      const moveY = local ? steering ? moving ? view.motion.moveY : input2.moveY : 0 : moving ? dy : 0;
      view.facing || (view.facing = new FacingState(view.row));
  ...
  renderEffects(effects) {
    for (const effect of effects) {
      ...
      if (effect.kind === "skillCast" || effect.kind === "bossPowerProc") {
        this.callouts || (this.callouts = new CombatCallouts(this.scene));
        this.callouts.show(effect, performance.now() / 1e3);
      }
      if (effect.kind === "skillCast" || effect.kind === "cinderZone") {
        this.skillEffects || (this.skillEffects = new SkillEffects(this.scene));
        this.skillEffects.show(effect, this.snapshotNow || 0, performance.now() / 1e3);
...
var AimView = class {
  ...
  update(playerView, input2, preview) {
    ...
    if (skill?.radius && skill.castType !== "zone") {
      const radius2 = skill.radius;
      g.lineStyle(2, color, 0.8).strokeCircle(origin.x, origin.y, radius2);
      g.fillStyle(color, 0.08).fillCircle(origin.x, origin.y, radius2);
      return;
    }
    ...
    const blocks = skill?.castType === "dash" ? (point) => structureBlocks(this.scene.views?.structures || [], point, PLAYER.radius) : void 0;
    ...
    if (skill?.castType === "fan") {
      ...
      for (let shot = 0; shot < skill.count; shot++) {
        ...
        g.lineStyle(14, color, 0.1).lineBetween(origin.x, origin.y, end.x, end.y);
        g.lineStyle(1, color, 0.75).lineBetween(origin.x, origin.y, end.x, end.y).strokeCircle(end.x, end.y, 7);
      }
      return;
    }
    g.lineStyle(radius * 2, color, 0.12).lineBetween(origin.x, origin.y, tip.x, tip.y);
    g.lineStyle(2, color, 0.8).lineBetween(origin.x, origin.y, tip.x, tip.y).strokeCircle(tip.x, tip.y, radius);
  }
};
  ...
  update(snapshot) {
    ...
    for (const effect of snapshot.effects || []) {
      ...
      if (effect.kind === "skillCast" && effect.ownerId === snapshot.you) {
        ...
        this.tone(frequencies[effect.skillId] || 440, 0.13, 0.028, "triangle");
      }
      if (effect.kind === "bossPower" && effect.ownerId === snapshot.you) this.tone(effect.power === "aegis" ? 740 : 360, 0.3, 0.035, "sine");
...
// src/sessions/SnapshotOrder.js
var PHASE = { select: 0, countdown: 1, playing: 2, finished: 3 };
var finite = (value) => Number.isFinite(value) ? value : null;
var SnapshotOrder = class {
  reset() {
    this.latest = null;
  }
  accept(snapshot) {
    const next = {
      ...
      now: finite(snapshot?.now),
      tick: finite(snapshot?.tick),
      phase: PHASE[snapshot?.match?.phase] ?? null
    };
...
// src/sessions/ChoiceTransport.js
var TYPES = /* @__PURE__ */ new Set(["upgrade", "relic", "reroll"]);
var isChoice = (type) => TYPES.has(type);
var failure = (code) => Object.assign(new Error(code), { code });
var ChoiceTransport = class {
  constructor(send, { now = Date.now, retryMs = 350, timeoutMs = 5e3 } = {}) {
    this.send = send;
    this.now = now;
    this.retryMs = retryMs;
    this.timeoutMs = timeoutMs;
    this.pending = /* @__PURE__ */ new Map();
    this.order = new SnapshotOrder();
  }
  request(type, data) {
  ...
  attempt(item) {
    ...
    if (this.now() - item.started >= this.timeoutMs) {
      this.finish(item, null, "CHOICE_TIMEOUT");
      return;
    }
    try {
      Promise.resolve(this.send(item.type, item.data)).catch(() => {
      });
    } catch {
    }
  ...
  observe(snapshot) {
    ...
    for (const item of [...this.pending.values()]) {
      const receipt = player.choiceReceipts?.find((value) => value.requestId === item.data.requestId && value.offerId === item.data.offerId && value.type === item.type && value.id === (item.data.id || null));
      if (receipt) this.finish(item, receipt, receipt.status === "applied" ? null : receipt.reason || "INVALID_CHOICE");
        ...
        const current2 = item.type === "relic" ? player.relicOffer?.id : player.offerId;
        if (current2 !== item.data.offerId) this.finish(item, null, "STALE_OFFER");
      }
    }
  }
  finish(item, receipt, code) {
    clearTimeout(item.timer);
    this.pending.delete(item.data.requestId);
    if (code) item.reject(Object.assign(failure(code), { receipt }));
    else item.resolve(receipt);
  }
  retry() {
    for (const item of [...this.pending.values()]) this.attempt(item);
  }
  stop() {
    for (const item of [...this.pending.values()]) this.finish(item, null, "CANCELLED");
    this.order.reset();
  }
};
...
function updateClones(world) {
  ...
  for (const clone of world.clones) {
    ...
    if (!target) continue;
    clone.nextShotAt = world.matchTime + config.cloneInterval;
    clone.shotsLeft -= 1;
...
var LocalSession = class {
  ...
  command(type, data = {}) {
    ...
    if (isChoice(type)) {
      ...
      return applied && receipt ? Promise.resolve(receipt) : Promise.reject(Object.assign(new Error(receipt?.reason || "INVALID_CHOICE"), { code: receipt?.reason || "INVALID_CHOICE" }));
    }
...
// src/ui/choiceFeedback.js
var COPY = {
  en: {
    pending: "Confirming choice\u2026",
    applied: "APPLIED",
    reroll: "New choices confirmed",
    unchanged: "Upgrade rank increased",
    stats: {
      hp: "HP",
      maxHp: "Max HP",
      basicDamage: "Attack damage",
      skillDamage: "Skill power %",
      basicReduction: "Basic defense %",
      skillReduction: "Skill defense %",
      speed: "Move speed",
      cooldown: "Skill cooldown reduction %",
      vision: "Vision range",
      structureDamage: "Structure damage bonus %",
      wardenShield: "Own-side shield",
      relicSeconds: "Duration (s)"
    },
    errors: {
      CHOICE_TIMEOUT: "Not confirmed yet. Tap again to retry safely.",
      STALE_OFFER: "That offer ended. Check the current choices.",
      INVALID_CHOICE: "Choice rejected. Choose an available option.",
      NOT_PLAYING: "Choices are unavailable now.",
      BUSY: "Confirming another choice\u2026"
    }
  },
  mn: {
    pending: "\u0421\u043E\u043D\u0433\u043E\u043B\u0442\u044B\u0433 \u0431\u0430\u0442\u0430\u043B\u0436 \u0431\u0430\u0439\u043D\u0430\u2026",
    applied: "\u0425\u042D\u0420\u042D\u0413\u0416\u041B\u042D\u042D",
    reroll: "\u0428\u0438\u043D\u044D \u0441\u043E\u043D\u0433\u043E\u043B\u0442 \u0431\u0430\u0442\u043B\u0430\u0433\u0434\u043B\u0430\u0430",
    unchanged: "\u0421\u0430\u0439\u0436\u0440\u0443\u0443\u043B\u0430\u043B\u0442\u044B\u043D \u0437\u044D\u0440\u044D\u0433 \u043D\u044D\u043C\u044D\u0433\u0434\u043B\u044D\u044D",
    stats: {
      hp: "\u0410\u043C\u044C",
      maxHp: "\u0414\u044D\u044D\u0434 \u0430\u043C\u044C",
      basicDamage: "\u0426\u043E\u0445\u0438\u043B\u0442\u044B\u043D \u0433\u044D\u043C\u0442\u044D\u043B",
      skillDamage: "Skill \u0445\u04AF\u0447 %",
      basicReduction: "\u0426\u043E\u0445\u0438\u043B\u0442\u044B\u043D \u0445\u0430\u043C\u0433\u0430\u0430\u043B\u0430\u043B\u0442 %",
      skillReduction: "Skill \u0445\u0430\u043C\u0433\u0430\u0430\u043B\u0430\u043B\u0442 %",
      speed: "\u0413\u04AF\u0439\u043B\u0442\u0438\u0439\u043D \u0445\u0443\u0440\u0434",
      cooldown: "Skill \u0445\u04AF\u043B\u044D\u044D\u043B\u0442 \u0431\u0443\u0443\u0440\u0430\u043B\u0442 %",
      vision: "\u0425\u0430\u0440\u0430\u0430\u043D\u044B \u0445\u04AF\u0440\u044D\u044D",
      structureDamage: "\u0426\u0430\u043C\u0445\u0433\u0438\u0439\u043D \u0433\u044D\u043C\u0442\u044D\u043B \u043D\u044D\u043C\u044D\u0433\u0434\u044D\u043B %",
      wardenShield: "\u04E8\u04E9\u0440\u0438\u0439\u043D \u0442\u0430\u043B\u044B\u043D \u0431\u0430\u043C\u0431\u0430\u0439",
      relicSeconds: "\u04AE\u0440\u0433\u044D\u043B\u0436\u043B\u044D\u0445 \u0441\u0435\u043A\u0443\u043D\u0434"
    },
    errors: {
      CHOICE_TIMEOUT: "\u0425\u0430\u0440\u0430\u0430\u0445\u0430\u043D \u0431\u0430\u0442\u043B\u0430\u0433\u0434\u0430\u0430\u0433\u04AF\u0439. \u0414\u0430\u0445\u0438\u043D \u0442\u043E\u0432\u0448\u0438\u0436 \u0430\u044E\u0443\u043B\u0433\u04AF\u0439 \u0434\u0430\u0432\u0442\u0430\u043D\u0430 \u0443\u0443.",
      STALE_OFFER: "\u04E8\u043C\u043D\u04E9\u0445 \u0441\u043E\u043D\u0433\u043E\u043B\u0442 \u0434\u0443\u0443\u0441\u0441\u0430\u043D. \u041E\u0434\u043E\u043E\u0433\u0438\u0439\u043D \u0441\u043E\u043D\u0433\u043E\u043B\u0442\u043E\u043E \u0445\u0430\u0440\u043D\u0430 \u0443\u0443.",
      INVALID_CHOICE: "\u0421\u043E\u043D\u0433\u043E\u043B\u0442 \u0437\u04E9\u0432\u0448\u04E9\u04E9\u0440\u04E9\u0433\u0434\u0441\u04E9\u043D\u0433\u04AF\u0439. \u0411\u043E\u043B\u043E\u043C\u0436\u0442\u043E\u0439 \u0445\u0443\u0432\u0438\u043B\u0431\u0430\u0440\u044B\u0433 \u0441\u043E\u043D\u0433\u043E\u043D\u043E \u0443\u0443.",
      NOT_PLAYING: "\u041E\u0434\u043E\u043E \u0441\u043E\u043D\u0433\u043E\u043B\u0442 \u0445\u0438\u0439\u0445 \u0431\u043E\u043B\u043E\u043C\u0436\u0433\u04AF\u0439.",
      BUSY: "\u04E8\u04E9\u0440 \u0441\u043E\u043D\u0433\u043E\u043B\u0442\u044B\u0433 \u0431\u0430\u0442\u0430\u043B\u0436 \u0431\u0430\u0439\u043D\u0430\u2026"
    }
  }
};
var choiceCopy = (language) => COPY[language] || COPY.en;
function receiptText(receipt, labels, language) {
  const copy2 = choiceCopy(language);
  if (receipt.type === "reroll") return `\u2713 ${copy2.reroll}`;
  const title = (receipt.type === "relic" ? labels.relics : labels.upgrades)[receipt.id]?.[0] || "";
  const details = Object.entries(receipt.benefits || {}).filter(([key, value]) => copy2.stats[key] && Number.isFinite(value.before) && Number.isFinite(value.after)).map(([key, value]) => `${copy2.stats[key]} ${value.before} \u2192 ${value.after}`);
  return `\u2713 ${copy2.applied} \xB7 ${title}${receipt.rank ? ` ${receipt.rank}` : ""}
${details.join(" \xB7 ") || copy2.unchanged}`;
}
...
var requestNumber = 0;
var newRequestId = () => globalThis.crypto?.randomUUID?.() || `choice:${Date.now()}:${++requestNumber}`;
var ChoiceView = class {
  constructor(command) {
    ...
    this.feedback.id = "choice-feedback";
    this.feedback.className = "choice-feedback is-hidden";
    this.feedback.setAttribute("role", "status");
    this.feedback.setAttribute("aria-live", "polite");
    $("#hud").append(this.feedback);
  }
  reset() {
    this.generation += 1;
    this.pending.clear();
    ...
    this.feedback.classList.add("is-hidden");
  }
  update(player, now, labels, language) {
    ...
    setText($("#build-summary"), ranks.map(([id, rank]) => `${labels.upgrades[id][0]} ${rank}`).join(" \xB7 "));
    ...
    setText($("#relic-status"), player.relic && seconds ? `${labels.relics[player.relic][0]} \xB7 ${seconds}s` : "");
  }
  reroll() {
    return this.choose("reroll", null, this.player?.offerId);
  }
  choose(type, id, offerId) {
    ...
    this.pending.set(offerId, { type, id });
    this.renderPending();
    this.show(choiceCopy(this.language).pending, "pending", 0);
    const generation = this.generation;
    let result;
    try {
      result = this.command(type, data);
    } catch (error) {
    ...
    return Promise.resolve(result).then((receipt) => {
      ...
      const copy2 = choiceCopy(this.language);
      this.show(copy2.errors[error?.code] || copy2.errors.INVALID_CHOICE, "error");
    }).finally(() => {
      if (generation !== this.generation) return;
  ...
  show(text, state, duration = 4500) {
    ...
    this.feedback.classList.remove("is-hidden");
  ...
  options(selector, ids, labels, type, offerId, ranks = {}) {
    ...
    root.replaceChildren(...ids.map((id) => {
      ...
      button.addEventListener("click", () => {
        if (!button.disabled) void this.choose(type, id, offerId);
      });
      return button;
    }));
  }
...
// src/ui/boss-copy.js
function bossName(power, language = "mn") {
  return power === "aegis" ? language === "mn" ? "\u25C6 \u0426\u042D\u041D\u0425\u042D\u0420 \u0425\u0410\u041C\u0413\u0410\u0410\u041B\u0410\u041B\u0422" : "\u25C6 BLUE AEGIS" : language === "mn" ? "\u2739 \u0423\u041B\u0410\u0410\u041D \u0426\u041E\u0425\u0418\u041B\u0422" : "\u2739 RED SURGE";
}
function bossDetail(power, language = "mn") {
  const config = BOSS_POWERS[power], mn = language === "mn";
  if (power === "aegis") return mn ? `\u0428\u0438\u043D\u044D\u044D\u0440 \u0430\u0448\u0438\u0433\u043B\u0430\u0445 skill-\u0438\u0439\u043D \u0445\u04AF\u043B\u044D\u044D\u043B\u0442 \u2212${Math.round(config.cooldownReduction * 100)}%; \u043E\u0434\u043E\u043E \u044F\u0432\u0436 \u0431\u0443\u0439 \u0445\u04AF\u043B\u044D\u044D\u043B\u0442\u0438\u0439\u0433 \u04E9\u04E9\u0440\u0447\u043B\u04E9\u0445\u0433\u04AF\u0439. \u0414\u0430\u0439\u0441\u043D\u044B \u0431\u0430\u0430\u0442\u0440\u044B\u043D \u0448\u0443\u0443\u0434 \u0446\u043E\u0445\u0438\u043B\u0442\u043E\u043E\u0441 ${config.guardDamage} \u0445\u04AF\u0440\u0442\u044D\u043B \u0433\u044D\u043C\u0442\u044D\u043B \u0445\u0430\u0430\u043D\u0430, ${config.guardCooldown}s \u0442\u0443\u0442\u0430\u043C.` : `New skill casts have \u2212${Math.round(config.cooldownReduction * 100)}% cooldown; existing cooldowns are unchanged. Block up to ${config.guardDamage} damage from a direct enemy hero hit, every ${config.guardCooldown}s.`;
  return mn ? `\u041E\u043D\u043E\u0441\u043E\u043D basic \u0446\u043E\u0445\u0438\u043B\u0442 +${config.hitDamage} \u0433\u044D\u043C\u0442\u044D\u043B, ${config.hitCooldown}s \u0442\u0443\u0442\u0430\u043C. \u0417\u04E9\u0432\u0445\u04E9\u043D \u0431\u0430\u0430\u0442\u0440\u044B\u0433 ${Math.round(config.slow * 100)}% \u0443\u0434\u0430\u0430\u0448\u0440\u0443\u0443\u043B\u043D\u0430 (${config.slowSeconds}s). \u0426\u0430\u043C\u0445\u0430\u0433\u0442 \u04AF\u0439\u043B\u0447\u043B\u044D\u0445\u0433\u04AF\u0439.` : `A landed basic hit adds ${config.hitDamage} damage, every ${config.hitCooldown}s. Only heroes are slowed by ${Math.round(config.slow * 100)}% for ${config.slowSeconds}s. Never affects structures.`;
}
function activeBossPowers(player, now, language = "mn") {
  if (player.spiritUntil > now) return [];
  return ["aegis", "tempo"].flatMap((power) => {
    ...
    return [{
      power,
      remaining,
      name: bossName(power, language),
      detail: bossDetail(power, language),
...
var AnnouncementState = class {
  ...
  update(snapshot, player, labels, language = "mn") {
    ...
    if (gained >= 0) message = `${bossName(gained ? "tempo" : "aegis", language)} \xB7 ${Math.ceil(untils[gained] - snapshot.now)}s`;
...
// src/ui/CombatReadability.js
var $2 = (id) => document.getElementById(id);
var CombatReadability = class {
  constructor() {
    this.open = false;
    this.buffNodes = ["aegis", "tempo"].map((power) => {
      const node = document.createElement("button");
      node.type = "button";
      node.className = `power-chip power-chip--${power}`;
      node.addEventListener("click", () => this.toggle(true));
      $2("boss-power-status").append(node);
      return node;
    });
    $2("combat-info").addEventListener("click", () => this.toggle(!this.open));
    $2("combat-info-close").addEventListener("click", () => this.toggle(false));
  ...
  toggle(open) {
    this.open = open;
    setClass($2("combat-inspect"), "is-hidden", !open);
    $2("combat-info").setAttribute("aria-expanded", String(open));
  }
  reset() {
    this.toggle(false);
  }
  update(player, now, language, labels) {
    ...
    $2("combat-info").setAttribute("aria-label", mn ? "Skill, \u0445\u04AF\u0447 \u0431\u0430 \u04AF\u0437\u04AF\u04AF\u043B\u044D\u043B\u0442 \u0445\u0430\u0440\u0430\u0445" : "Inspect skills, powers and stats");
    $2("combat-info-close").setAttribute("aria-label", mn ? "\u0425\u0430\u0430\u0445" : "Close");
    const powers = activeBossPowers(player, now, language);
    setClass($2("boss-power-status"), "is-hidden", !powers.length);
    this.buffNodes.forEach((node, index) => {
      const power = powers.find((item) => item.power === (index ? "tempo" : "aegis"));
      setClass(node, "is-hidden", !power);
      if (power) {
        setText(node, `${power.name} \xB7 ${power.remaining}s \xB7 ${power.readiness}`);
        node.title = power.detail;
        node.setAttribute("aria-label", `${node.textContent}. ${power.detail}`);
      }
    });
    if (!this.open) return;
    const stats = derivedStats(player, now), round3 = (value) => Math.round(value * 10) / 10;
    setText($2("combat-inspect-title"), `${labels.heroes[player.hero][0]} \xB7 ${mn ? "\u0422\u043E\u0433\u043B\u043E\u043B\u0442 \u04AF\u0440\u0433\u044D\u043B\u0436\u0438\u043B\u043D\u044D" : "Match stays live"}`);
    setText($2("combat-stats"), `${mn ? "\u0426\u043E\u0445\u0438\u043B\u0442" : "Attack"} ${round3(stats.basicDamage)} \xB7 HP ${stats.maxHp} \xB7 ${mn ? "\u0425\u04E9\u0434\u04E9\u043B\u0433\u04E9\u04E9\u043D\u0438\u0439 \u0445\u0443\u0440\u0434" : "Move speed"} ${round3(predictionSpeed(player, now))} \xB7 Skill \xD7${stats.skillDamage.toFixed(2)}`);
    setText($2("combat-defense"), `${mn ? "\u0425\u0443\u044F\u0433 / \u0441\u0430\u0445\u0438\u0443\u0441" : "Basic / skill reduction"} ${round3(stats.basicReduction * 100)}% / ${round3(stats.skillReduction * 100)}%`);
    setText($2("combat-ranges"), `${mn ? "\u0426\u043E\u0445\u0438\u043B\u0442\u044B\u043D \u0437\u0430\u0439" : "Attack ranges"}: ${mn ? "\u0431\u0430\u0430\u0442\u0430\u0440" : "hero"} ${PLAYER.attackRange} \xB7 tower ${STRUCTURES.tower.range} \xB7 core ${STRUCTURES.core.range} \xB7 ${mn ? "\u0446\u044D\u0440\u044D\u0433" : "minions"} ${MINIONS.melee.range}/${MINIONS.ranged.range}/${MINIONS.siege.range}. ${mn ? "\u0426\u0430\u043C\u0445\u0430\u0433\u0438\u0439\u0433 \u0437\u04E9\u0432\u0445\u04E9\u043D \u0445\u04AF\u0440\u044D\u044D\u043D \u0434\u043E\u0442\u0440\u043E\u043E\u0441 \u043D\u044C \u0446\u043E\u0445\u0438\u043D\u043E." : "Structures can only be hit from inside their threat ring."}`);
    for (let index = 0; index < 2; index++) {
      setText($2(`combat-skill-${index}`), `${index ? "E" : "Q"} \xB7 ${skillVerb(HEROES[player.hero].skills[index].id, language)} \u2014 ${skillLiveDetail(player, index, stats, language)}`);
    }
    setText($2("combat-passive"), passiveDetail(player.hero, language));
    setText($2("combat-powers"), ["aegis", "tempo"].map((power) => `${bossName(power, language)}: ${bossDetail(power, language)}`).join("\n"));
    setText($2("combat-power-rule"), mn ? "30s \u04AF\u0440\u0433\u044D\u043B\u0436\u0438\u043B\u043D\u044D. \u0414\u0430\u0445\u0438\u043D \u0430\u0432\u0431\u0430\u043B \u0445\u0443\u0433\u0430\u0446\u0430\u0430 \u043B \u0448\u0438\u043D\u044D\u0447\u043B\u044D\u0433\u0434\u044D\u043D\u044D; \u04AF\u0445\u0432\u044D\u043B \u0430\u0440\u0438\u043B\u043D\u0430. \u0425\u043E\u0451\u0440 \u0431\u043E\u0441\u0441\u044B\u0433 \u0443\u043D\u0430\u0433\u0430\u0432\u0430\u043B \u043D\u044D\u043C\u044D\u043B\u0442 relic \u0441\u043E\u043D\u0433\u043E\u043D\u043E." : "Lasts 30s. Repeated kills refresh duration only; death removes powers. Defeat both bosses on one half for an extra relic choice.");
  }
};

// src/ui/UIController.js
var $3 = (selector) => document.querySelector(selector);
...
var UIController = class {
  constructor(language = "mn") {
    ...
    $3("#language").addEventListener("click", () => this.setLanguage(this.language === "mn" ? "en" : "mn"));
    $3("#draft-retry").addEventListener("click", () => this.callbacks.retry?.());
    $3("#draft-action").addEventListener("click", () => this.handleLobbyAction());
    $3("#reroll").addEventListener("click", () => this.choices.reroll());
    $3("#practice-again").addEventListener("click", () => this.callbacks.finish?.(this.mode));
    this.renderHeroes();
    this.applyLanguage();
  }
  on(name, callback) {
  ...
  applyLanguage() {
    const t = this.t();
    $3("#boot-copy").textContent = t.boot;
    $3("#hero-title").textContent = t.choose;
    $3("#select-status").textContent = t.waiting;
    $3("#blue-label").textContent = t.you;
    $3("#red-label").textContent = t.rival;
    $3("#upgrade-title").firstChild.textContent = `${t.upgrade} `;
    $3("#relic-title").firstChild.textContent = `${t.relic} `;
    $3("#reroll").textContent = t.reroll;
    $3("#practice-again").textContent = t.again;
    $3("#result-hint").textContent = t.hint;
    $3("#language").textContent = this.language === "mn" ? "EN" : "MN";
    $3("#draft-auto").textContent = t.lobbyRule;
    $3("#draft-retry").textContent = t.retry;
    $3("#game-guide").textContent = t.guide;
    $3("#move-copy").textContent = t.move;
    $3("#fire-copy").textContent = t.fire;
    $3("#farm-copy").textContent = t.farm;
    $3("#structure-copy").textContent = t.structure;
    $3("#priority-copy").textContent = t.target;
    for (const [id, hint] of [["aim-stick", t.attackHint], ["attack-farm", t.farmHint], ["attack-structure", t.structureHint], ["target-priority", t.priorityHint]]) {
      $3(`#${id}`).title = hint;
      $3(`#${id}`).setAttribute("aria-label", hint);
    }
    for (const option of $3("#target-priority").options) option.textContent = t.priorities[option.value];
    $3("#skill-guide").textContent = t.skillGuide;
    if (this.currentHero) this.selectHero(this.currentHero);
    this.updateDraft(this.lastDraft);
  }
  renderHeroes() {
    const grid = $3("#hero-grid");
  ...
  ready(mode, state = mode === "network" ? this.networkState : "solo") {
    $3("#boot-screen").classList.remove("screen--active");
  ...
  selectHero(hero) {
    ...
    $3("#skill-1-name").textContent = skillVerb(HEROES[hero].skills[0].id, this.language);
    $3("#skill-2-name").textContent = skillVerb(HEROES[hero].skills[1].id, this.language);
    $3("#hero-kit").textContent = this.t().skillDetails[hero].join("  \u2022  ");
    this.t().skillDetails[hero].forEach((detail, index) => {
      const button = $3(`#skill-${index + 1}`);
      button.title = `${detail}
${this.t().skillGuide}`;
  ...
  setPhase(phase) {
    ...
    $3("#hero-screen").classList.toggle("screen--active", visible.draft);
    $3("#hud").classList.toggle("is-hidden", !visible.battle);
    $3("#controls").classList.toggle("is-hidden", !visible.controls);
    $3("#choice-stack").classList.toggle("is-hidden", !visible.choices);
    $3("#result-screen").classList.toggle("screen--active", visible.result);
  }
  resetSession() {
    this.choices.reset();
    ...
    $3("#upgrade").classList.add("is-hidden");
    $3("#relic").classList.add("is-hidden");
    this.updateDraft(null);
  }
  setNetwork(mode, state = "ready") {
    this.mode = mode;
    this.networkState = state;
    const node = $3("#network");
    ...
    $3("#draft-retry").classList.toggle("is-hidden", mode !== "network" || state !== "poor");
    this.updateDraft(this.lastDraft);
  }
  handleLobbyAction() {
  ...
  updateDraft(snapshot) {
    ...
    const connection = $3("#draft-connection");
    ...
    $3("#draft-auto").textContent = this.mode === "network" ? t.lobbyRule : t.practiceRule;
    $3("#draft-roster").textContent = this.mode === "network" ? `${you?.name || t.you}${you?.host ? ` \xB7 ${t.host}` : ""} \xB7 ${playerState(you)}   VS   ${rival?.name || t.rival}${rival?.host ? ` \xB7 ${t.host}` : ""} \xB7 ${rivalState}` : `${t.you} \xB7 ${selected ? t.ready : t.pick}   VS   BOT \xB7 ${t.ready}`;
    ...
    $3("#select-status").textContent = status;
    const action = $3("#draft-action");
  ...
  update(snapshot) {
    ...
    setText($3("#clock"), `${String(Math.floor(seconds / 60)).padStart(2, "0")}:${String(seconds % 60).padStart(2, "0")}`);
    setText($3("#wave"), `${this.t().wave} ${snapshot.match.wave}`);
    setText($3("#blue-label"), you.name || this.t().you);
    setText($3("#red-label"), rival?.name || this.t().rival);
    setText($3("#blue-level"), `LV ${you.level}`);
    setText($3("#red-level"), `LV ${rival?.level || 1}`);
    setStyle($3("#blue-core"), "width", pct(ownCore.hp / ownCore.maxHp));
    setStyle($3("#red-core"), "width", pct(rivalCore.hp / rivalCore.maxHp));
    setText($3("#objective-status"), `${this.t().nextWave} ${Math.max(0, Math.ceil(snapshot.match.nextWaveAt - snapshot.now))}s \xB7 ${this.t().bossPair} ${Math.max(...you.guardianProgress || [0])}/2`);
    setText($3("#hero-name"), this.t().heroes[you.hero]?.[0]?.toUpperCase() || "HERO");
    setText($3("#hp-copy"), `${Math.ceil(you.hp)} / ${Math.ceil(you.maxHp)}`);
    setStyle($3("#hp-bar"), "width", pct(you.hp / you.maxHp));
    setStyle($3("#shield-bar"), "width", pct((you.shield || 0) / you.maxHp));
    setStyle($3("#shield-bar"), "left", "0px");
    setStyle($3("#xp-bar"), "width", pct(xpProgress(you).ratio));
    ...
    drawMinimap($3("#minimap"), snapshot);
  ...
  showResult(snapshot, you) {
    ...
    $3("#result-title").textContent = this.t()[result];
    $3("#result-stats").textContent = `${you.kills} KILLS \xB7 ${you.deaths} DEATHS \xB7 LV ${you.level}`;
    $3("#practice-again").textContent = this.mode === "network" ? this.t().exit : this.t().again;
    $3("#result-hint").textContent = this.mode === "network" ? this.t().networkResult : this.t().hint;
    $3("#result-reason").textContent = this.t().finishReasons[snapshot.match.finishReason] || "";
    $3("#result-screen").classList.add("screen--active");
    $3("#controls").classList.add("is-hidden");
  }
  toast(message) {
    const node = $3("#toast");
...
// src/ui/interaction-guard.js
var installed = /* @__PURE__ */ new WeakMap();
function installGameInteractionGuard(documentObject = document) {
